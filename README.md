# GJ Warfare 🎮

Welcome to the official **GJ Warfare** website and repository!  
This project is a **Pixel-style 3D Shooter Game** built with the Ursina Engine in Python.


## 🔫 Play the Game
The game is available as an **exe**.  
Download the file from our site and run - works on any computer

## 🌐 Website
Visit the official site: [gj-warfare.github.io](https://gj-warfare.github.io)

------------

## 📜 License
This project is released under the MIT License for Github and is owned by GJ Warfare
